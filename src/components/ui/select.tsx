...
const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, children, ...props }, ref) => {
    return (
      <select
        className={cn(
          "flex h-9 w-full appearance-none rounded-md border border-border bg-input bg-[length:1rem] bg-[right_0.5rem_center] bg-no-repeat px-3 py-1 pr-8 text-sm text-foreground shadow-sm transition-colors",
