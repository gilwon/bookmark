...
export const ITEMS = [
  {
    ...
    description:
      "히어로 섹션 레퍼런스를 모아 둔 곳이다. 첫 화면을 어떻게 시작할지 막히면 여기서 보면 된다.",
    tags: ["히어로", "레퍼런스"],
  },
  {
    ...
    description:
      "네비게이션 패턴만 모아 둔 사이트다. 헤더가 홈 인상을 많이 좌우하니 구조를 잡을 때 보면 쉽다.",
    tags: ["내비", "헤더", "레퍼런스"],
  },
  {
    title: "Footer Design",
    ...
    description:
      "푸터 레퍼런스 모음이다. 마지막이 힘 빠진 사이트와 달리 잘 만든 푸터 사례를 볼 수 있다.",
    tags: ["푸터", "레퍼런스"],
  },
];
...
/** 같은 URL의 기존 행을 찾아 저장된 주소를 반환한다. */
export function findExistingUrl(existingRows, item) {
...
function insertLocal(items, now) {
  ...
  try {
    ...
    const favorite = db.prepare(
      `UPDATE bookmarks SET is_favorite = 1 WHERE user_id = ? AND url = ?`
    );
    const add = db.transaction(() => {
      ...
      for (const item of skippedDup) {
        ...
        favorite.run(LOCAL_USER, existingUrl);
      }
    });
    add();
    return {
