...
export const PAGE_TITLE = "클로드 다크 프롬프트 10";
export const SOURCE_URL =
  "https://slashpage.com/biggie-ai/d367nxm3wr133mj98pv1";
export const PAGE_HASH = "d367nxm3wr133mj98pv1";
export const EXPECTED_IMAGES = 2;
export const EXPECTED_ATTACHMENTS = 0;
export const EXPECTED_TABLES = 0;
export const EXPECTED_CODES = 10;
const EXPECTED_IMAGE_BYTES = {
  "image/slashpagePost/20260916/162838_F1F6nF8LbbJDNvHLcU": 2479100,
  "image/slashpagePost/20260916/163215_QSkuVbrKl4x2TEwCT2": 269,
};
...
const CODE_PHRASE = "잔인할 정도로 솔직하게 말해";
const REQUIRED_LINKS = [
  "https://product.kyobobook.co.kr/detail/S000221330983",
  "https://forms.gle/gqpvf9dotkWDfL5s9",
];
const IMAGE_ALTS = ["header.png", "divider.png"];
const OTHER_PAGE_TITLES = [
  "ChatGPT 이미지 프롬프트 99개",
  "ChatGPT 프롬프트 해킹 99개",
];
const OTHER_PAGE_HASHES = ["1q3vdn2pdpnk82xy49pr", "3p4kj92yjdq9ym57q1x8"];
...
const RESIZE_QUERY_PARTS = ["upload.cafenono.com", "q=", "s=", "t=", "f="];
...
/** tokenLink가 비어 있으면 clickAction value의 url을 읽는다. 파싱 실패는 링크 없음이다. */
function clickActionUrl(token) {
  const extended = token?.extended;
  if (!extended || extended.type !== "clickAction") return "";
  let value = extended.value;
...
export function tokensToMarkdown(tokens) {
  ...
  return tokens
    .map((token) => {
      ...
      const href = tokenLink(token) || clickActionUrl(token);
...
function assertIntegrity({ markdown, content, stats, blockMap }) {
  ...
  if (OTHER_PAGE_TITLES.includes(PAGE_TITLE) || OTHER_PAGE_HASHES.includes(PAGE_HASH)) {
    throw new Error("다른 Slashpage 글과 섞였습니다.");
  }
  ...
  if (!markdown.includes(CODE_PHRASE)) {
    throw new Error(`코드 문구가 없습니다. ${CODE_PHRASE}`);
  }
  for (const alt of IMAGE_ALTS) {
    if (!markdown.includes(`![${alt}](`)) {
      throw new Error(`이미지 대체 텍스트가 없습니다. ${alt}`);
    }
  }
  for (const link of REQUIRED_LINKS) {
    if (!markdown.includes(link) || !content.includes(link)) {
      throw new Error(`링크가 없습니다. ${link}`);
    }
  }
  const images = allBlocks(blockMap).filter((block) => block.type === "image");
  if (images.length !== EXPECTED_IMAGES) {
    throw new Error(`이미지 블록 수가 다릅니다. ${images.length}`);
  }
  for (const block of images) {
    const image = block.value?.image || {};
    if (block.parentBlockId !== "_root_") {
      throw new Error(`이미지가 본문 루트가 아닙니다. ${image.filename ?? ""}`);
    }
    if (isChromeImage(image)) {
      throw new Error(`작성자 이미지가 본문에 있습니다. ${image.filename ?? ""}`);
    }
    const expected = EXPECTED_IMAGE_BYTES[image.imageKey];
    if (!expected || Number(image.originalContentLength) !== expected) {
      throw new Error(`이미지 기대 바이트가 다릅니다. ${image.imageKey ?? ""}`);
    }
    if (!IMAGE_ALTS.includes(image.filename)) {
      throw new Error(`이미지 파일명이 다릅니다. ${image.filename ?? ""}`);
    }
  }
  ...
  if (countType(blockMap, "code") !== EXPECTED_CODES) {
    throw new Error(`코드 블록 수가 다릅니다. ${countType(blockMap, "code")}`);
  }
  ...
  for (const part of RESIZE_QUERY_PARTS) {
...
async function downloadOriginalImage(image) {
  ...
  const declared = Number(image.originalContentLength);
  const expected = EXPECTED_IMAGE_BYTES[image.imageKey];
  if (!expected || bytes.length !== expected || bytes.length !== declared) {
    throw new Error(
      `이미지 바이트 수가 다릅니다. ${image.imageKey ?? ""} ${bytes.length} ${declared}`
    );
  }
  if (
    bytes[0] !== 0x89 ||
...
async function resolveMedia(blockMap) {
  // 대표 이미지는 본문 header.png와 같은 블록이라 메타 경로를 다시 받지 않는다.
  const media = new Map();
