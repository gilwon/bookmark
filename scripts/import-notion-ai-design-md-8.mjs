...
const SPACE_ID = "bc51061c-8a63-811c-bf7d-000388d15787";
...
const GITHUB_README_URL =
  "https://github.com/nexu-io/open-design/blob/main/docs/i18n/README.ko.md";
const GITHUB_README_LABEL = "OpenDesign 한글 README";
...
const REQUIRED_HREFS = [
  ...
  GITHUB_README_URL,
  "https://open.kakao.com/o/glKSlnqi",
  "https://youtube.com/@aimekorea?si=JyjUAoD74aKqAXr2",
  "https://www.designtokens.org/",
];
const EXPECTED_ROOT_CHILDREN = 81;
const EXPECTED_CODES = 1;
const EXPECTED_TABLES = 7;
const EXPECTED_TOGGLES = 1;
const EXPECTED_BLOCKS = 205;

export const SOURCE_URL =
  "https://app.notion.com/p/AI-DESIGN-md-8-3db1061c8a6380c699a1c2feb2c2e9c8";
export const PAGE_TITLE =
  "AI한테 \"예쁘게 만들어줘\" 그만하세요 — DESIGN.md 리소스 8곳";
export const NOTION_PAGE_ID = "3db1061c-8a63-80c6-99a1-c2feb2c2e9c8";
export const PAGE_HEX = "3db1061c8a6380c699a1c2feb2c2e9c8";
export const EXPECTED_IMAGES = 8;
...
/** 이미 링크가 아닌 맨 https URL을 마크다운 링크로 바꾼다. */
  ...
  return String(markdown).replace(
    ...
    (match, prefix, href, suffix, bare) => {
      if (bare) {
        ...
        return `[${bare}](${stripTracking(bare)})`;
      }
...
/** 외부 객체와 할 일 블록을 마크다운이 알아먹는 형태로 바꾼다. */
function preprocessBlocks(blocks) {
  for (const block of blocks.values()) {
    if (block.type === "external_object_instance") {
      const url = block.format?.original_url || "";
      if (url === GITHUB_README_URL) {
        block.properties = {
          ...(block.properties || {}),
          title: [[GITHUB_README_LABEL, [["a", url]]]],
        };
        const parent = getBlock(blocks, block.parent_id);
        if (parent && !(parent.content ?? []).includes(block.id)) {
          parent.content = [...(parent.content ?? []), block.id];
        }
      }
    }
    if (block.type === "to_do") {
...
function finalizeMarkdown(markdown) {
  ...
  if (text.startsWith("# ")) {
    ...
    text = `# ${PAGE_TITLE}${newline === -1 ? "" : text.slice(newline)}`;
  }
...
function assertIntegrity({ page, blocks, pageTitle, markdown, stats, content }) {
  ...
  if (blocks.size !== EXPECTED_BLOCKS) {
    throw new Error(`블록 수가 다릅니다. ${blocks.size}`);
  }
  if (missingChildCount(blocks, NOTION_PAGE_ID) !== 0) {
    throw new Error("빠진 자식 블록이 있습니다.");
  }
  if (countType(blocks, "image") !== EXPECTED_IMAGES) {
  ...
  if (imageSrcs.length !== EXPECTED_IMAGES) {
  ...
  if (countType(blocks, "table") !== EXPECTED_TABLES) {
  ...
  if (countType(blocks, "toggle") !== EXPECTED_TOGGLES) {
    throw new Error(`토글 수가 다릅니다. ${countType(blocks, "toggle")}`);
  }
  for (const phrase of REQUIRED_PHRASES) {
    if (!markdown.includes(phrase)) throw new Error(`문구가 없습니다. ${phrase}`);
  }
  for (const href of [SOURCE_URL, ...REQUIRED_HREFS]) {
...
async function main() {
  ...
  const markdown = finalizeMarkdown(built.markdown);
  ...
  const extra = {
    ...
    toggles: countType(blocks, "toggle"),
    blocks: blocks.size,
  };
  if (checkOnly) {
