...
const SPACE_ID = "eafdd631-56c6-4737-af2a-43a630bd6895";
...
const REQUIRED_PHRASES = [
  "사용 방법",
  "01. 카메라 앵글 (001–010)",
  "15. 컬러그레이딩 & 필터 (141–150)",
  "사진 한 장 + 코드 한 줄로",
  "ChatGPT에 원하는 사진을 업로드한 뒤",
  "/lowangle",
  "/highangle",
  "/tealorange",
  "/blackwhite",
];
const EXPECTED_ROOT_CHILDREN = 37;
const EXPECTED_TABLES = 15;
const EXPECTED_TABLE_ROWS = 165;
const EXPECTED_COMMANDS = 150;
const EXPECTED_SUB_HEADERS = 16;

export const SOURCE_URL =
  "https://app.notion.com/p/GPT-150-3d289afb51cb804ab06cf1b0f2f66702";
export const PAGE_TITLE = "챗GPT 이미지 생성 치트키 150개";
export const NOTION_PAGE_ID = "3d289afb-51cb-804a-b06c-f1b0f2f66702";
export const PAGE_HEX = "3d289afb51cb804ab06cf1b0f2f66702";
...
/** 표 헤더를 뺀 슬래시 코드 행 수다. */
export function commandCount(blocks) {
  let count = 0;
  for (const block of blocks.values()) {
    if (block.type !== "table_row") continue;
    const cells = Object.values(block.properties ?? {}).map((value) =>
      plainText(value).trim()
    );
    if (cells.includes("코드") && cells.includes("설명")) continue;
    if (cells.some((cell) => /^\/[A-Za-z0-9]/.test(cell))) count += 1;
  }
  return count;
}
...
/** 표 행이 165라 loadPageChunk 커서를 모두 돈다. */
...
function assertIntegrity({ page, blocks, pageTitle, markdown, stats, content }) {
  ...
  if (countType(blocks, "table_row") !== EXPECTED_TABLE_ROWS) {
    throw new Error(`표 행 수가 다릅니다. ${countType(blocks, "table_row")}`);
  }
  if (commandCount(blocks) !== EXPECTED_COMMANDS) {
    throw new Error(`명령어 수가 다릅니다. ${commandCount(blocks)}`);
  }
  if (stats.tables !== EXPECTED_TABLES) {
    throw new Error(`TipTap 표 수가 다릅니다. ${stats.tables}`);
  }
  if (countType(blocks, "sub_header") !== EXPECTED_SUB_HEADERS) {
    throw new Error(`소제목 수가 다릅니다. ${countType(blocks, "sub_header")}`);
  }
...
async function main() {
  ...
  const extra = {
    ...
    tables: stats.tables,
    tableRows: countType(blocks, "table_row"),
    commands: commandCount(blocks),
    subHeaders: countType(blocks, "sub_header"),
    images: stats.images,
