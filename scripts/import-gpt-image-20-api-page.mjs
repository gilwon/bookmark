// GPT IMAGE 2.0 API 투명 배경 사용 가이드 Notion 원문을 Pages에만 저장한다
...
export const SOURCE_URL =
  "https://app.notion.com/p/GPT-IMAGE-2-0-API-3c3fd99f0e5f81e7a70fe48d445fd5cd";
export const NOTION_PAGE_ID = "3c3fd99f-0e5f-81e7-a70f-e48d445fd5cd";
...
export const EXPECTED_TITLE = "GPT IMAGE 2.0 API 투명 배경 사용 가이드";
const EXPECTED_ROOT_CHILDREN = 76;
const EXPECTED_TOGGLES = 3;
...
export function renderBlock(block, blocks, media = new Map(), path = new Set()) {
  ...
  if (type === "code") {
    ...
    const raw =
      ...
      "text";
    const lang = /^plain\s*text$/i.test(raw) ? "text" : String(raw).toLowerCase();
    return `\`\`\`${lang}\n${body}\n\`\`\``;
  }
...
function assertIntegrity({ page, blocks, pageTitle, markdown, stats }) {
  ...
  if ((page.content || []).length !== EXPECTED_ROOT_CHILDREN) {
    throw new Error(`루트 자식 수가 76이 아닙니다. ${(page.content || []).length}`);
  }
  if (toggleCount(blocks) !== EXPECTED_TOGGLES) {
    throw new Error(`토글 수가 3이 아닙니다. ${toggleCount(blocks)}`);
  }
  if (imageCount(blocks) !== EXPECTED_IMAGES) {
    throw new Error(`이미지 블록 수가 4가 아닙니다. ${imageCount(blocks)}`);
  }
  if (!markdown.includes(SOURCE_URL)) throw new Error("원문 주소가 없습니다.");
  if (!markdown.includes("```")) throw new Error("코드 펜스가 없습니다.");
  if (!markdown.includes("client.images.generate")) {
    throw new Error("Python 예제 호출이 없습니다.");
  }
  if (!markdown.includes('background="transparent"')) {
    throw new Error("투명 배경 옵션이 없습니다.");
  }
  if (!markdown.includes("바닥 그림자와 반사 없이 인물만")) {
    throw new Error("게임 프롬프트 문구가 없습니다.");
  }
  for (const header of REQUIRED_HEADERS) {
    if (!markdown.includes(header)) throw new Error(`헤더가 없습니다. ${header}`);
  }
  if (stats.images !== EXPECTED_IMAGES) {
    throw new Error(`TipTap 이미지 수가 4가 아닙니다. ${stats.images}`);
  }
  if (stats.tables !== EXPECTED_TABLES) {
    throw new Error(`표 수가 6이 아닙니다. ${stats.tables}`);
  }
