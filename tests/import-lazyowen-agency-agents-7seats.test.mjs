// 게으른 빌더 에이전트 일곱 자리 가이드 이관 헬퍼를 네트워크 없이 검증한다
...
const SCRIPT_PATH = resolve(
  ...
  "../scripts/import-lazyowen-agency-agents-7seats.mjs"
);

const FIXTURE = `<!doctype html>
<html lang="ko">
  <head>
    <title>무료 AI 에이전트 명단에서 먼저 깔아야 할 일곱 자리 · 게으른 빌더</title>
  </head>
  <body>
    ...
    <article class="article">
      <header class="article-head">
        <h1>무료 AI 에이전트 명단에서 먼저 깔아야 할 일곱 자리</h1>
      </header>
      <div class="prose">
        <div class="callout">
          <p>공개된 무료 AI 에이전트 명단에서 일곱 자리만 고릅니다.</p>
        </div>
...
git clone --depth 1 https://github.com/msitarzewski/agency-agents.git ~/agency-agents</code></pre>
            <button type="button" class="copy">복사</button>
          </div>
        </details>
        <p>공식 GitHub: <a href="https://github.com/msitarzewski/agency-agents?utm_source=share&amp;fbclid=IwAR123">msitarzewski/agency-agents</a></p>
        <p>인스타: <a href="https://www.instagram.com/lazy_owen/">게으른 빌더</a></p>
        <p><img src="/guides/agency-agents-7seats/g1-flow.webp" alt="명단을 받아 일곱 자리를 고르고 설치해 총괄에게 목표를 던지기까지의 전체 흐름"></p>
        <div class="tablewrap">
          <table>
            <thead><tr><th>자리</th><th>설치할 때 적는 이름</th></tr></thead>
            <tbody><tr><td>총괄 매니저</td><td>agents-orchestrator</td></tr></tbody>
          </table>
        </div>
        <p>공식 사이트는 <a href="https://agencyagents.app/">agencyagents.app</a>입니다.</p>
      </div>
    </article>
...
test("스크립트 첫 줄은 한글 역할 주석이다", () => {
  ...
  assert.equal(
    source.split("\n")[0],
    "// 게으른 빌더 에이전트 일곱 자리 가이드를 Pages에만 저장한다"
  );
});
...
test("parseLazyowenHtml은 제목·원문 인용·본문만 남긴다", () => {
  ...
  assert.equal(
    parsed.markdown.includes("https://github.com/msitarzewski/agency-agents"),
    true
  );
